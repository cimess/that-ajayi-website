  ...
  return (
    <div className="min-h-screen bg-eko-black text-white font-sans">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-eko-black/95 backdrop-blur-sm border-b border-white/10 p-4 flex items-center justify-between">
        <span><img src="/logo/thatAjayi-Logo.webp" alt="" className="w-10 h-10 rounded-full" /></span> <h2 className="text-xl font-serif font-bold text-eko-gold">That-Ajayi Agency</h2>
        <button
      ...
      <div className="flex pt-16 lg:pt-0">
        ...
        <aside className={`
          ...
          lg:pt-6 pt-20
        `}>
          <span className="flex items-center gap-2 mb-10"><img src="/logo/thatAjayi-Logo.webp" alt="" className="w-10 h-10 rounded-full" />
            <h2 className="text-2xl font-serif font-bold text-white ">That-Ajayi</h2></span>
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-3 mb-8 bg-white/5 p-2 rounded-2xl w-fit">
                    {(['all', 'pending', 'approved', 'rejected'] as const).map((status) => (
                      <button
                        key={status}
                        onClick={() => setFilter(status)}
                        className={`
        px-6 py-2 rounded-xl text-sm font-medium transition-all duration-300 capitalize
        ${filter === status
                            ? 'bg-eko-gold text-eko-black shadow-lg shadow-eko-gold/20'
      ...
      `}
                  ...
                  {submissions.length === 0 ? <p className="text-gray-400">No submissions yet.</p> : (
                    submissions.filter(sub =>filter === "all" || sub.status === filter).map(sub => (
                      <div key={sub.id} className="bg-white/5 border border-white/10 p-6 rounded-xl flex gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`text-xs px-2 py-1 rounded-full ${sub.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' :
                      ...
                      <div key={booking.id} className="bg-white/5 border border-white/10 p-6 rounded-xl flex justify-between items-start">
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`text-xs px-2 py-1 rounded-full ${booking.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' :
